        ...
        try {
            if (!location.href.includes('form-templates')) {
                ...
                if (logTitle === 'VALIDATED_REQUIRED_FIELD_IDS') {
                    ...
                    if (JotForm.isEditMode()) {
                        errorData['submissionID'] = $$('input[name="editSubmission"]')[0].value;
                    } else {
                        errorData['eventID'] = $$('input[name="event_id"]')[0].value;
                    }
                }
    ...
    init: function (callback) {
        var ready = function () {
            try {
                ...
                // We are seeing form submissions with all answers, (except for default values) with empty values. They
                // are also not being submitted via the submit button. We want to rule out forms being submitted via
                // HTMLFormElement.submit(). To do this we're adding a log to the submit method
                if (jotformForm) {
                    // Monkey patch submit to include tracking as onsubmit won't be fired
                    jotformForm.submit = (function (originalSubmitFn) {
                        return function () {
                            JotForm.trackSubmitEvent(jotformForm, 'direct');
                ...
                this.questionCount = document.querySelectorAll('.form-line').length;
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                ...
                var uploader = JotForm.isFillingOffline() ? false : new qq.FileUploader({
                    ...
                    onComplete: function (id, filename, response) {
                        ...
                        if (response.success) {
                            ...
                            var tmpFolder = this.params.folder;
                            var uploadHiddenID = [tmpFolder, qFolder, filename].join('_');
                            ...
                            // Call endpoint to check if the file we just uploaded still exists after 10 seconds. We 
                            // suspect there may be an issue with how files are handled. This endpoint will log to 
                            // Kibana if the file is missing.
                            if ($(uploadHiddenID)) {
                                ...
                                var fileStatusUrl = JotForm.getAPIEndpoint() + '/submission/fileuploadstatus/' + formID + '?' + queryParams;

                                JotForm.createXHRRequest(fileStatusUrl, 'GET', function() {}, function() {});
                            }
                        }
                    },
    ...
    setFocusEvents: function () {
        ...
        if ((JotForm.newPaymentUI) && window.paymentType === 'product' && window.FORM_MODE !== 'cardform') {
            ...
            $$('.form-checkbox').each(function (element) {
                var productItem = element.up('.form-product-item');
        ...
        $$('[name$=\[month\]]').each(function (monthElement) {
            ...
            monthElement.dateTimeCheck = function (e) {
                ...
                if (!erroredElement) {
                    ...
                    if (liteDateInput && liteDateInput.hasClassName('invalidLimits')) {
                        erroredElement = liteDateInput;
                        limitError = true;
                    }
                }
    ...
    prepareCalculationsOnTheFly: function (questions) {
        JotForm.errorCatcherLog({}, 'PREPARE_CALCULATIONS_ON_THE_FLY');
    ...
    hideField: function (field, multiple, dontClear) {
        ...
        if (element) {
            ...
            if (element.getAttribute('data-type') && element.getAttribute('data-type') == 'control_button' && element.getElementsBySelector('button').length > 1) {
                ...
                if (saclButton) {
                    // this meas that the form has a save & continue later
    ...
    clearField: function (field, subfield, dontTrigger) {
        ...
        if(type === "matrix" && subfield && $(subfield)) {
            ...
            if (type == "checkbox" || type == "radio") {
                ...
                if ($('id_' + field).triggerEvent && !dontTrigger && dataChanged) $('id_' + field).triggerEvent('change');
            } else if (type == "select") {
                if ($('input_' + field)) {
                    $('input_' + field).value = defaultValue;
   ...
   */
    ...
    applyCoupon: function (discount) {
        ...
        var displayOldPrice = function (container, id) {
            ...
            var lineThroughStyle = window.FORM_MODE === 'cardform' ? '' : 'text-decoration:line-through;';
            var span = new Element('span', {style: lineThroughStyle + 'display:inline-block;'}).addClassName('old_price');
    ...
    handlePages: function () {
        ...
        $$('.form-pagebreak').each(function (page, i) {
            ...
            function stopEnterKey(evt) {
                ...
                if (evt.keyCode == 13 && ["text",  "email", "tel", "number", "radio", "checkbox", "select-one", "select-multiple"].include(node.type)) {
                    return false;
                }
            ...
            section.select('.form-pagebreak-back').invoke('observe', 'click', function () { // When back button is clicked
                ...
                prevPage.select('.form-line[data-type=control_widget]').each(function (e) {
    ...
    showFormSection: function (section, withoutFocus) {
        ...
        if (firstLabel && !firstLabel.up('.form-section-closed') && !withoutFocus) {
            ...
            if (inputElementID){
                ...
                var isIosSelectBox = inputElement && JotForm.browserIs.ios() && inputElement.classList.contains('form-dropdown');
                const isIframeOnIOS = JotForm.browserIs.ios() && window !== window.top;
                if (inputElement && !isIframeOnIOS) {
                    var focusInputElement = function() {
                        inputElement.focus();
                    };
            
                    if (isIosSelectBox) {
                        setTimeout(focusInputElement);
                    } else {
                        focusInputElement();
                    }
                }
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        function handleValidateAll(form, scopeSelector) {
            ...
            var allValidateClasses = $(scopeSelector).querySelectorAll('*[class*="validate"]')

            Array.prototype.forEach.call(allValidateClasses, function(input) {
            ...
            form.observe('submit', function (e) {
                try {
                    ...
                    if (JotForm.validatedRequiredFieldIDs != null && typeof JotForm.validatedRequiredFieldIDs === 'object') {
                        if (Object.keys(JotForm.validatedRequiredFieldIDs).length > 0) {
                    ...
                    if (Object.keys(JotForm.visitedPages).length > 0) {
                        form.insert(new Element('input', {type: 'hidden', name: 'visitedPages'}).putValue(JSON.stringify(JotForm.visitedPages)));
                    }
                ...
                JotForm.trackSubmitEvent(form, 'form');
  ...
  /**
    ...
    /**
     * @desc Track form submission events.
     * @param form {HTMLFormElement}
     * @param source {String}
     */
    trackSubmitEvent: function (form, source) {
        // Check for forEach support (ie 8)
        if (Array.prototype.forEach) {
            var formData = {};
            var formIDElement = document.querySelector('input[name="formID"]');
            var eventIDElement = document.querySelector('input[name="event_id"]');
            var logKey = source === 'form' ? 'BLANK' : 'EMPTY';

            // Get the form data as an object, this method is safer than querying the FormData API due to compatibility
            Array.prototype.forEach.call(form.elements || [], function (element) {
                if (element.tagName !== 'BUTTON') formData[element.name] = {
                    hasValue: !!element.value,
                    required: !!element.required,
                    disabled: element.disabled && !element.hasClassName('conditionallyDisabled')
                }
            });

            // Send event to Kibana
            ...
            }, (logKey + '_SUBMISSION_TRACK_FE'));

            // Because we can't trust Kibana to track the above log, add a query param to the form submit action, so we can
            // know where the submission event came from.
            form.action += (form.action.indexOf('?') > -1 ? '&' : '?') + 'submit_source=' + source;
        }
    }
};
